(function () {
    ...
    Bundle = new Class({
        ...
        "initialize": function (params) {
			...
			that.params = params;
			that.params.searchString = "•" + that.params.tab + "•" + that.params.group + "•";

			that.createDOM();
			that.setupDOM();
			that.addEvents();
            
            if (that.params.id !== undefined) {
				that.element.set("id", that.params.id);
            }
            
            //if (that.params.name !== undefined) {
				//settings.get(that.params.name, function (item){
				//	"use strict";
			//
				//	that.set(item[that.params.name], true);
				//});
            //}
